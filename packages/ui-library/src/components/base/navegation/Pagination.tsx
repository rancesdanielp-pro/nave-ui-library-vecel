...
function Pagination({
  ...
  const mergedTokens = resolveTokens({ componentName: 'pagination', tokens: customTokens }, theme) ?? {};

  const styles = {
    '--pg-active-bg': mergedTokens?.activeBackground,
    '--pg-active-text': mergedTokens?.activeText,
    '--pg-border-color': mergedTokens?.border,
    '--pg-hover-bg': mergedTokens?.hoverBackground,
  ...
  return (
    <nav
      ...
      style={styles}
      className={cn('mx-auto flex w-full justify-center group/pagination', className)}
      {...props}
    />
  );
}
...
/* -----------------------------------------------------------------------------
 * Resto de sub-componentes (Sin cambios)
 * ---------------------------------------------------------------------------*/
...
function PaginationEllipsis({ className, ...props }: React.ComponentProps<'span'>) {
  return (
    <span data-slot="pagination-ellipsis" className={cn('inline-flex items-center justify-center w-9 h-9 text-slate-400', className)} {...props}>
